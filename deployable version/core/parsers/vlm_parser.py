...
def _call_page(client, cfg: Config, b64_url: str, is_first: bool) -> str:
    prompt = (
    ...
    with ThreadPoolExecutor(max_workers=cfg.vlm_max_workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.warning("VLM page %d error for %s: %s", idx + 1, path.name, exc)
...
def parse(path: Path, cfg: Config) -> ParseResult:
    try:
        ...
        logger.exception("VLM failed for %s", path.name)
